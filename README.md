...
为了确保工作流能正确检索影片信息，仓库名必须严格遵循：**影片官方英文名_年份**（例如：`Cosmos_Laundroma_2015`）
...
工作流会自动根据仓库名在 TMDB / 豆瓣检索影片信息，并将本 README 重写为正式的项目主页。

> [!NOTE]
> - 自动化流程需要仓库为公开状态才能正常读写仓库信息。
> - 如果仓库名格式不正确或未在 TMDB 中匹配到影片，工作流会中止并给出错误说明，请按提示重新创建仓库。
...
To ensure the workflow can correctly retrieve movie information, the repository name must strictly follow this format: **Official English Title_Year** (e.g., `Cosmos_Laundroma_2015`).
...
The workflow automatically retrieves movie metadata from TMDB or Douban based on the repository name and rewrites this README as the official project homepage.
