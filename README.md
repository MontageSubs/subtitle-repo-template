...
1. 检查本仓库当前的可见性：页面顶部仓库名右侧会有一个标签，显示 **Public** 或 **Private**
   - 已显示 **Public** → 直接进行下一步
   - 显示 **Private**（或其他非 Public 状态）→ 点击 [此链接](../../settings#danger-zone) 直达 Danger Zone，或手动前往 **Settings → General → Danger Zone**，选择 **Change repository visibility → Public**
...
1. Check this repository's current visibility: a label next to the repository name at the top of the page shows either **Public** or **Private**
   ...
   - Shows **Private** (or anything other than Public) → click [this link](../../settings#danger-zone) to jump directly to Danger Zone, or manually go to **Settings → General → Danger Zone**, then select **Change repository visibility → Public**
